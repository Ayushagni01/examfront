...
interface NewsArticle {
  ...
  content: string
  summary: string | null
  category: 'ADMIT_CARD' | 'RESULT' | 'NOTIFICATION' | 'VACANCY' | 'ANSWER_KEY' | 'SYLLABUS' | 'GENERAL'
...
export default function ManageNews() {
  ...
  const loadNews = () => {
    setLoading(true)
    api.get(`/admin/news?page=${page}&size=${size}`)
      .then(res => {
        setNews(res.data.content || [])
        setTotalPages(res.data.totalPages || 0)
        setTotalElements(res.data.totalElements || 0)
      })
      .catch(() => setMsg({ text: 'Failed to load news articles', type: 'error' }))
  ...
  const openEdit = (n: NewsArticle) => {
    ...
    setForm({
      ...
      imageUrl: n.imageUrl || '',
      publishedDate: n.publishedDate ? n.publishedDate.substring(0, 10) : new Date().toISOString().substring(0, 10),
      examId: n.exam ? String(n.exam.id) : '',
  ...
  const save = async () => {
    ...
    const payload = {
      ...
      exam: form.examId ? { id: Number(form.examId) } : null,
    ...
    try {
      if (editing) {
        ...
        setMsg({ text: 'News article updated successfully!', type: 'success' })
      } else {
        await api.post('/admin/news', payload)
        setMsg({ text: 'News article created successfully!', type: 'success' })
      }
      setModal(false)
      loadNews()
    } catch (err: any) {
      setMsg({ text: err.response?.data?.message || 'Error saving news', type: 'error' })
    } finally {
      setSaving(false)
    }
  }
  ...
  const del = async () => {
    if (!delId) return
    try {
      await api.delete(`/admin/news/${delId}`)
      setMsg({ text: 'News article deleted successfully', type: 'success' })
      setDelId(null)
      loadNews()
    } catch {
      setMsg({ text: 'Failed to delete news article', type: 'error' })
      setDelId(null)
    }
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Manage News & Notifications</h1>
          <p className="text-slate-500 text-sm mt-1">{totalElements} articles total</p>
        </div>
          ...
          <>
            <table className="w-full text-sm">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  {['Title', 'Category', 'Exam Tag', 'Published Date', 'Status', 'Actions'].map(h => (
              ...
              <tbody className="divide-y divide-slate-100">
                {news.map(n => (
                  <tr key={n.id} className="hover:bg-slate-50 transition-colors">
                    <td className="px-4 py-3 font-medium text-slate-800 max-w-md truncate">{n.title}</td>
                    <td className="px-4 py-3 text-slate-500">{n.category}</td>
                    <td className="px-4 py-3 text-slate-500">{n.exam?.name || 'N/A'}</td>
                    <td className="px-4 py-3 text-slate-500">{n.publishedDate}</td>
                    <td className="px-4 py-3">
                      <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${
                        n.isActive ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                      }`}>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <button onClick={() => openEdit(n)} className="p-1.5 hover:bg-slate-100 rounded-lg text-slate-500 hover:text-indigo-600 transition-colors">
                          <Pencil size={15} />
                        </button>
                        <button onClick={() => setDelId(n.id)} className="p-1.5 hover:bg-red-50 rounded-lg text-slate-500 hover:text-red-600 transition-colors">
                          <Trash2 size={15} />
                        </button>
                      </div>
                    </td>
                ...
                {news.length === 0 && (
                  <tr>
                    <td colSpan={6} className="px-4 py-8 text-center text-slate-400">No articles found</td>
                  </tr>
                )}
              </tbody>
            </table>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 overflow-y-auto">
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-white rounded-xl w-full max-w-2xl shadow-2xl my-8">
              <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200">
                <h2 className="text-lg font-bold text-slate-800">{editing ? 'Edit News Article' : 'Add News Article'}</h2>
        ...
        {delId && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-white rounded-xl w-full max-w-sm shadow-2xl p-6 text-center">
              ...
              <h3 className="text-lg font-bold text-slate-800 mb-1">Delete Article?</h3>
